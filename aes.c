...
inline void aes_gen_tables(void)
{
}
	...
	*SK++ = *RK++;
	...
	for (i = 1; i < ctx->nr; i++) {
        ...
        *SK++ = KT0[(uint8_t) (*RK >> 24)] ^
            KT1[(uint8_t) (*RK >> 16)] ^
            KT2[(uint8_t) (*RK >> 8)] ^ KT3[(uint8_t) (*RK)];
		RK++;
