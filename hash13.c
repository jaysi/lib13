...
#include "include/hash13i.h"
#include "include/end13.h"

#define HASH_LITTLE_ENDIAN  LIB13_LITTLE_ENDIAN
#define HASH_BIG_ENDIAN     LIB13_BIG_ENDIAN
...
static inline uint8_t GetCRC8(uint8_t crc, uint8_t ch)
{
    int i;
...
uchar h13_pearson(uint8_t *key, size_t len)
{
    uchar hash;
    size_t i;
    for (hash = len, i = 0; i < len; ++i)
        hash = pearsontab[hash ^ key[i]];
    return (hash);
}
